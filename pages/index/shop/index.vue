					...
					<view class="flex justify-between">
						<view style="width: 500rpx;">
							...
							<view class="flex align-center margin-top-xs">
								...
								<view class="margin-left-xs" v-if="shopDet.errandTime">配送约{{shopDet.errandTime}}分钟
								</view>
							</view>
						...
						<view class="padding-tb-sm margin-lr u-border-bottom" v-for="(item, index) in EvaluateList"
							...
							<view class="flex justify-between align-center">
								<view class="flex align-center">
									...
									<view class=" margin-left-sm" style="line-height: 46upx;">{{item.nickName?item.nickName:'匿名'}}</view>
			...
			getOrderList() {
				...
				this.$Request.get("/app/order/selectAllOrderList", data).then(res => {
					if (res.code == 0 && res.data.pageUtils.list.length) {
						console.log(res.data.pageUtils.list, '752')
