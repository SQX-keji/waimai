	...
	<view class="pages">
		...
		<view class="food">
			...
			<view class="tosend_header_do justify-between do_bot" @click="isShow">
				...
				<view v-else><image src="../../../static/images/order/right1.png" style="width: 14rpx;height: 24rpx;" mode=""></image></view>
			</view>
		...
		<view class="margin-top padding-lr radius bg-white" style="width: 94%;margin: 0 auto;border-radius: 18rpx;">
			...
			<view class="flex align-center justify-between padding-tb" style="height: 100upx;"
				...
				<view class="flex-sub text-lg margin-left">{{item.text}}</view>
	...
	export default {
		...
		methods: {
			...
			getCouponList() {
				...
				let minMoney = this.orderType == 2 ? parseFloat(this.totalPrice * 1).toFixed(2) * 1 + this.paotuiMoney * 1 + this.dabaoMoney * 1 : parseFloat(this.totalPrice * 1).toFixed(2)
				let data = {
					page: 1,
					limit: 10,
					minMoney: minMoney
			...
			getOrderList() {
				...
				this.$Request.get("/app/order/selectAllOrderList", data).then(res => {
					...
					if (res.code == 0&&res.data.pageUtils.list) {
						...
						this.totalPrice = res.data.money * 1
